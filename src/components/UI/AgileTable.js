...
import {
    ...
    fetchComplianceStatusData, FileToBase64,
    shortName
} from "../../Assests/CommonFunctions";

const AgileTable = (props) => {
    const { headings, dataToSendAPI } = props;
    const { complianceId, projectId, accountId, sowId, stageId, sprintId } = dataToSendAPI;
    ...
    const getQuestions = async () => {
        setModifiedData([]);
        setQuesLoading(true)
        const { data } = await axiosCalls.get(`/Account/GetAgileQuestions?ComplianceTypeId=${complianceId}&ProjectId=${projectId}&AccountId=${accountId}&SowId=${sowId}&StageId=${stageId}&SprintId=${sprintId}`);
        data.map((d, i) => {
            d["queNo"] = i + 1;
            d["uniqueKey"] = i;
            return d;
    ...
    const submitData = async () => {
        ...
        if (valid) {
            onlyChangedData.forEach(async (chanedData, i) => {
                setLoading(true);
                const mergedData = { ...dataToSendAPI, ...chanedData };
                const sendThisData = {
                    ...
                    "isUploaded": mergedData.isUploaded,
                    "artefactId": mergedData.artefactId ? mergedData.artefactId : 0
                }

                try {
                    const { data } = await axiosCalls.post('Account/SaveAgileQuestionResponse', sendThisData);
                    if (data.status) {
    ...
    const deleteFile = async (artefactIdToDelete) => {
        ...
        if (result) {
            setLoading(true)
            const data = await axiosCalls.delete(`Account/DeleteSprintQuestionResponse?ArtefactId=${artefactIdToDelete}`);
            if (data.status === 204) {
    ...
    useEffect(() => {
        if (complianceId && projectId && accountId && sowId && stageId && sprintId) {
            fetchComplianceStatusData(setComplianceStatusData)
            fetchCommentTypeData(setCommentTypeData)
            getQuestions();
        }
    }, [complianceId, projectId, accountId, sowId, stageId, sprintId]);

...
const ProjectAllocation2 = () => {
    ...
    //useForm states
    const { control, handleSubmit, register, formState: { errors }, reset, setValue, watch, getValues } = useForm({
        mode: "onSubmit"
    });
    ...
    const getDefaultData = async () => {
        ...
        const bothEmpContractor = arraySort([...modifiedEmps, ...modifiedContractEmps], "text")
            .filter(({ value: id1 }) => !projectAllocationData.some(({ employeeId: id2 }) => id2 === id1));
    ...
    const setDateData = (d1, d2) => {
        ...
        // setDefaultData({ ...defaultData, sowStartDate: d1, sowEndDate: d2 });
    }
    ...
    const editProjectAllocation = async (eData) => {
        ...
        setDisValues(eData.employeeId, eData);
        setOpen(true);
    }
    ...
    const setDisValues = async (eId, updateData = {}) => {
        try {
            let edit = Object.keys(updateData).length > 0 ? 1 : 0
            setFormLoading(true);
            setAllDisable(false);
            // reset({})
                                            ...
                                            return (
                                                <Table.Row key={i}>
                                                    ...
                                                    <Table.Cell>
                                                        <button
                                                            className="pl-1"
                                                            onClick={() => editProjectAllocation(mapData)}
                                                            disabled={sowResponse.isActive === 0}
                                                        >
                                                            <Icon
                                                                color='teal'

...
const ProjectAllocation2 = () => {
    ...
    const getDefaultData = async () => {
        ...
        const empWithAllocationDate = bothEmpContractor.map(b => {
            projectAllocationData.map(c => {
                if (b.value === c.employeeId && c.isActive === 1) {
                    b.projectOnBoardingDate = c.projectOnBoardingDate
                    b.projectOffBoardingDate = c.projectOffBoardingDate
                } else {
                    b.projectOnBoardingDate = ""
                    b.projectOffBoardingDate = ""
                }
            })
            return b;
        })

        setEmployeeList(empWithAllocationDate);
    ...
    const changeEmployeeOnDates = () => {
        
        let list;
        const sd = new Date(sDate);
        const ed = new Date(eDate);
        
        if (sd > ed) return alert("OYEEE")
        else {
            list = employeeList.filter(emp => {
                const { projectOnBoardingDate, projectOffBoardingDate } = emp;
                if (projectOnBoardingDate === "" && projectOffBoardingDate === "") {
                    return emp;
                } else {
                    if (
                        (sd < new Date(projectOnBoardingDate) && ed < new Date(projectOnBoardingDate))
                        ||
                        (sd > new Date(projectOffBoardingDate) && ed > sd)
                    ) {
                        return emp;
                    }
                }
        ...
        setStateData({ ...stateData, employeeData: list });
    }
            ...
            if (edit) {
                ...
                setDefaultData(a => {
                    return {
                        ...
                        utiStatusDisable: emData.allocatedBillability > 0 ? true : false,
                    }
                })
                ...
                <Modal.Content >
                    ...
                    <Form onSubmit={handleSubmit(onSubmit)} loading={formLoading}>
                            ...
                            <Form.Group widths={"equal"}>
                                <Form.Field required>
                                    ...
                                    <Form.Dropdown
                                        ...
                                        disabled={allocatedBillDisable || !empId}
                                ...
                                <Form.Field required>
                                    ...
                                    <Form.Dropdown
                                        ...
                                        disabled={allocatedUtiDisable || !empId}
                                        key={changeKey}
                                ...
                                <Form.Field required>
                                    ...
                                    <Form.Dropdown
                                        ...
                                        key={utiStatusShow}
                                        disabled={utiStatusDisable || !empId}
                                        placeholder={
                        ...
                        <Modal.Actions>
                            ...
                            <Button type="submit" content={isEdit ? "Update" : "ADD"} positive disabled={allDisable || !empId} />
